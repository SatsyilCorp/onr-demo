...
the paused file-arrival job, the Score job, and the SDP pipeline. No deploy button.
"""
...
_PACKAGED_EXCERPT = """\
# Databricks Asset Bundle — onr_demo / target dev
...
def render_bundle() -> None:
    ...
    st.markdown("### Bundle")
    st.caption(
        f"`databricks bundle deploy -t dev` · `{label}` · read-only. "
        "Project repository. Full file is in the Full bundle tab."
    )
